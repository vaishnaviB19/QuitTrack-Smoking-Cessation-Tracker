...
const CigaretteLogs = () => {
  ...
  //Autofill last added savings  
  useEffect(() => {
  const lastPrice = localStorage.getItem("lastCigPrice");
  if (lastPrice) {
    setFormData((prev) => ({ ...prev, pricePerCigarette: lastPrice }));
  }
}, []);
 ...
 const handleSubmit = async (e) => {
  ...
  if (!formData.quantity || !formData.pricePerCigarette) {
    return alert("Please enter quantity and price!");
  }
  ...
  try {
    ...
    const res = await axiosInstance.post(
      "/api/logs",
      {
        ...
        pricePerCigarette: Number(formData.pricePerCigarette),
      },
      {
        headers: { Authorization: `Bearer ${token}` },
      }
    ...
    localStorage.setItem("lastCigPrice", formData.pricePerCigarette);
